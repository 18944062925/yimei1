...
function getCardIdInfor(command) {
    ...
    $.ajax({
        url: "/register/"+$("#cardId").val(),
        type: "get",
        contentType: 'json',
        success: function (da) {
            if (da !== null && da.code == 0) {
                registerID=da.rows.id;
                console.log(da.rows);
                // if (null == data.message) {
                $("#cardId").val(da.rows.registeredNum);
                $("#name").val(da.rows.name);
                $("#sex").val(da.rows.sex);
                $("#nationality").val(da.rows.nationality);
                $("#age").val(da.rows.age)
            } else {
                swal(da.msg, "", "error")
            }
...
function addPatientInfor() {
    ...
    $.ajax({
        ...
        success: function (da) {
            ...
            if (da !== null && da.code == 0) {
                setTimeout(function () {
                    swal("办理成功！卡号为:" + data.registerednum, "", "success");
                    cancel();
...
window.operateEvents = {
    ...
    "click #btn_register": function (e, value, row, index) {
        ...
        ({
                title: "确认选择吗?",
                text: "医生:<span style='color: #2C9FAF'>" + username + "</span>",
                html: true,
                showCancelButton: true,
            },
            function () {
                $("#treatmentPrice").val(allow_num)
            })
    }
};
...
function addRegisterInfor() {
    ...
    ({
        ...
        text: "卡号:<span style='color: #2C9FAF'>" + cardId + "</span>&emsp;科室:<span style='color: #2C9FAF'>" + departmentName + "</span>" +
            "<br>类型:<span style='color: #2C9FAF'>" + registerTypeName + "</span>&emsp;医生:<span style='color: #2C9FAF'>" + username + "</span>" +
            "<br>地址:<span style='color: #2C9FAF'>" + work_address + "</span>",
        type: "info",
        html: true,
        showCancelButton: true,
        closeOnConfirm: false,
        ...
        $.ajax({
            ...
            data: {
                ...
                doctor: doctor,
                payType: $(".payType").val(),
                registerType: $("#departmentTypeSelect").val(),
                price: $("#treatmentPrice").val(),
                registerPrice: $("#registerPriice").val(),
                payPrice: $("#payMoney").val(),
                changePrice: $("#Change").val()
            },
            success: function (data) {
